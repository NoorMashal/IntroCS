public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        //Extract information from the string "description"
        //System.out.println(description.substring(description.indexOf("name:") + 5, description.indexOf(";", description.indexOf("name:"))));
        String name = description.substring(description.indexOf("name:")+5, description.indexOf(";", description.indexOf("name:")));
        String type = description.substring(description.indexOf("type:")+5, description.indexOf(";", description.indexOf("type:")));
        double due = Double.parseDouble(description.substring(description.indexOf("due:")+4, description.indexOf(";", description.indexOf("due:"))));
        double monthly = Double.parseDouble(description.substring(description.indexOf("monthly:")+8, description.indexOf(";", description.indexOf("monthly:"))));
        int length = Integer.parseInt(description.substring(description.indexOf("length:")+7, description.indexOf(";", description.indexOf("length:"))));
        double usage = Double.parseDouble(description.substring(description.indexOf("mile/unit:")+10, description.indexOf(";", description.indexOf("mile/unit:"))));
        int mileageAllowance = Integer.parseInt(description.substring(description.indexOf("allowance:")+10, description.indexOf(";", description.indexOf("allowance"))));
        ...
        if(description.indexOf("charger:") > 0){
          int charger = Integer.parseInt(description.substring(description.indexOf("charger:")+8, description.indexOf(";", description.indexOf("charger:"))));
        ...
        //create lease
        Lease lease = new Lease(due, length, monthly, mileageAllowance);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        for (int i = 0; i < vehicles.length; i++){
          ...
          double milageAllowance = vehicles[i].getLease().getMileageAllowance();

          if(vehicles[i].getFuel().getType() == 1){
            vehicles[i].setCO2Emission(computeCO2(numberOfMonth, usage, milageAllowance, 8.887 ));
            vehicles[i].setFuelCost(computeFuelCost(numberOfMonth, usage, milageAllowance, gasPrice));
            vehicles[i].setTotalCost(computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), (int)numberOfMonth, vehicles[i].getLease().getMonthlyCost())+vehicles[i].getFuelCost());
          }
          else{
            vehicles[i].setCO2Emission(computeCO2(numberOfMonth, usage, milageAllowance, 0.453 ));
            vehicles[i].setFuelCost(computeFuelCost(numberOfMonth, usage, milageAllowance, electricityPrice));
            vehicles[i].setTotalCost(computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), (int)numberOfMonth, vehicles[i].getLease().getMonthlyCost())+vehicles[i].getFuelCost()+vehicles[i].getFuel().getCharger());
          }
