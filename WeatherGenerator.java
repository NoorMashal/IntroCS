...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator(double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        for (int i = 0; i < numberOfDays; i++) {
          if (prob < 0.5){
            forecast[i] = WET;
            prob = wetwetProbability;
          }
          else{
            forecast[i] = DRY;
            prob = drywetProbability;
          }
        }
        return forecast;
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        double dryProb = drywetProbability[month];
        double wetProb = wetwetProbability[month];

        int[] monthForecast = forecastGenerator(dryProb, wetProb, numOfDays);

        /*for (int i = 0; i < monthForecast.length; i++){
          StdOut.print(monthForecast[i]+" ");
        }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        int count = 0;   // Total number of mode
        ...
        //StdOut.println("Count of "+mode+": "+count);
        //StdOut.println("Result of "+mode+": "+result);
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        int result = 0; // Maximum
        int count = 0;   // Total number of mode
        int index = 0; // Index of the first day of the 7 days period with the most dry days in forecast
